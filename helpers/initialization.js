...
const registerUser = async () => {
  ...
  let confirmPasswordElement = await driver.findElement(
...
const addBook = async () => {
  await driver.wait(until.titleContains('Add a book'), 3000);
  ...
  let numberOfPagesElement = await driver.findElement(By.name('numberOfPages'));
  await numberOfPagesElement.sendKeys('412', Key.TAB);
...
const editBook = async () => {
  await driver.wait(until.titleContains('Edit a book'), 3000);
  let authorElement = await driver.findElement(By.id('author'));
  ...
  await ISBNElement.sendKeys('127-980-457-5', Key.TAB);
  let numberOfPagesElement = await driver.findElement(By.name('numberOfPages'));
  await numberOfPagesElement.clear();
  await numberOfPagesElement.sendKeys('444', Key.TAB);
  let priceElement = await driver.findElement(By.name('price'));
...
const addAuthor = async () => {
  await driver.wait(until.titleContains('Add an author'), 3000);
  sendPayload('name', 'Steve Irwin');
  sendPayload('publisher', 'Aussie Publishing');
  sendPayload('website', 'www.australiazoo.au');
...
const editAuthor = async () => {
  await driver.wait(until.titleContains('Edit an author'), 2000);
